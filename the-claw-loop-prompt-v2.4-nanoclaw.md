...
- The recipient needs NanoClaw installed and running with at least one messaging channel
- The target project must be mounted into the container via `additionalMounts` with `readonly: false`
- The mount allowlist (`~/.config/nanoclaw/mount-allowlist.json`) must permit read-write for the project path
- Container timeout should be at least 30 minutes (`containerConfig.timeout: 1800000`)
- The project must have BMAD V6 slash commands in `.claude/commands/`
- The project must have `_bmad-output/implementation-artifacts/sprint-status.yaml` (run `/bmad-bmm-sprint-planning` first)
- The prompt walks the NanoClaw agent through setup step by step
- Works with any messaging channel NanoClaw supports (WhatsApp, Telegram, Slack, Discord, etc.)

## Key Differences from tmux-based Version

| Aspect | tmux Model (ClawdBot/original) | Direct CLI Model (NanoClaw) |
|--------|-------------------------------|----------------------------|
| Worker process | Persistent Claude Code in tmux session | Fresh `claude -p` per step |
| Context management | `/clear` between steps | Automatic — each invocation starts clean |
| Model switching | `/model opus` sent to tmux | `--model opus` flag on CLI |
| Stall detection | Pane fingerprinting across cycles | Exit code + timeout |
| Observation | `tmux capture-pane` every 3 min | Parse stdout after step completes |
| Interaction model | Check/nudge every 3 min | Run full step per fire |
| Step transitions | Send next command to running session | Update state, next fire runs next step |
| Recovery | Kill/restart tmux session | Just re-run the step (fresh process) |
| Dependencies | tmux installed on host | Claude CLI in container (already installed) |
| Socket/session management | tmux socket path, session name | None needed |
| Smart-skip thresholds | 2-5 min (frequent checks) | 4-8 min (full step execution) |

## Key Differences from Previous Versions

| Feature | v2.4-tmux | v2.4-nanoclaw |
|---------|-----------|---------------|
| Architecture | Orchestrator checks persistent worker | Orchestrator runs worker per step |
| Token cost per fire | ~800 chars (lightweight ping) | ~800 chars (same lightweight ping) |
| Context management | Manual `/clear` between steps | Automatic (fresh process) |
| Failure recovery | Complex stall tiers (T1-T4) | Simple exit code + retry count |
| Cron procedure | 5 steps with pane capture/analysis | 4 steps — resolve, run, parse, report |
| tmux dependency | Required on host | None |
| Container compatibility | Requires socket mount | Native — runs inside container |
